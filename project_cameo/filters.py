...
class VCovolutionFilter(object):
    """A filter that apply covolution to V"""
...
class SharpenFilter(VCovolutionFilter):
    """A sharpen filter with 1-pixel radius"""
    def __init__(self):
        kernel=numpy.array([[-1,-1,-1],
                            [-1,9,-1],
                            [-1,-1,-1]])
        VCovolutionFilter.__init__(self,kernel)


class FindEdgeFilter(VCovolutionFilter):
...
class BlurFilter(VCovolutionFilter):
    """
    A blur filter with a 2-pixel radius
    For a blur effect, the weights should sum up to 1 and shoud be positive throughout
    the neighborhood.
    """
    def __init__(self):
...
class EmbossFilter(VCovolutionFilter):
    """
    blurs on one side(with positive weights)
    sharpens on the other side(with negative weights)
    """
    def __init__(self):
